...
DASHBOARD_APP_NAME="Azure - Week 3"
...
# -------------------------------
# Azure Metadata Helper
# -------------------------------
azure_md() {
  curl -fsS --connect-timeout 2 --max-time 3 \
  -H "Metadata:true" \
  "http://169.254.169.254/metadata/instance$1?api-version=2021-02-01" 2>/dev/null | jq -r "$2" 2>/dev/null || echo "unknown"
}
...
# -------------------------------
# Install jq for JSON parsing (Azure metadata requires it)
# -------------------------------
wait_for_apt
...
retry apt-get install -y \
  nginx python3 curl ca-certificates git build-essential
...
# -------------------------------
# Azure Metadata Collection
# -------------------------------
log "Collecting Azure metadata"

# Azure metadata requires jq for JSON parsing
# First get the full metadata
AZURE_METADATA=$(curl -fsS -H "Metadata:true" \
  "http://169.254.169.254/metadata/instance?api-version=2021-02-01" 2>/dev/null)

HOSTNAME_VM="$(hostname)"
INSTANCE_ID=$(echo "$AZURE_METADATA" | jq -r '.compute.vmId' 2>/dev/null || echo "unknown")
VM_SIZE=$(echo "$AZURE_METADATA" | jq -r '.compute.vmSize' 2>/dev/null || echo "unknown")
LOCATION=$(echo "$AZURE_METADATA" | jq -r '.compute.location' 2>/dev/null || echo "unknown")
RESOURCE_GROUP=$(echo "$AZURE_METADATA" | jq -r '.compute.resourceGroupName' 2>/dev/null || echo "unknown")
SUBSCRIPTION_ID=$(echo "$AZURE_METADATA" | jq -r '.compute.subscriptionId' 2>/dev/null || echo "unknown")
OFFER=$(echo "$AZURE_METADATA" | jq -r '.compute.offer' 2>/dev/null || echo "unknown")
SKU=$(echo "$AZURE_METADATA" | jq -r '.compute.sku' 2>/dev/null || echo "unknown")
VERSION=$(echo "$AZURE_METADATA" | jq -r '.compute.version' 2>/dev/null || echo "unknown")

# Get IPs
INTERNAL_IP=$(echo "$AZURE_METADATA" | jq -r '.network.interface[0].ipv4.ipAddress[0].privateIpAddress' 2>/dev/null || hostname -I | awk '{print $1}' 2>/dev/null || echo "unknown")
PUBLIC_IP=$(echo "$AZURE_METADATA" | jq -r '.network.interface[0].ipv4.ipAddress[0].publicIpAddress' 2>/dev/null || curl -s ifconfig.me 2>/dev/null | tr -d '\n' || echo "unknown")
...
# Export for Python
# -------------------------------
export HOSTNAME_VM INSTANCE_ID VM_SIZE LOCATION RESOURCE_GROUP SUBSCRIPTION_ID
export OFFER SKU VERSION INTERNAL_IP PUBLIC_IP OS_NAME UPTIME
...
def get_cost_estimate_azure():
    """Azure-specific cost estimation"""
    try:
        vm_size = os.environ.get('VM_SIZE', 'Standard_B1s').lower()
        cpu = float(os.environ.get('CPU_USAGE', '0'))
        
        # Azure B series pricing (US East)
        if "b1s" in vm_size:
            base_hourly = 0.012
        elif "b1ms" in vm_size:
            base_hourly = 0.024
        elif "b2s" in vm_size:
            base_hourly = 0.048
        elif "b2ms" in vm_size:
            base_hourly = 0.096
        else:
            base_hourly = 0.035
        ...
        return f"\${total:.2f}/month (Azure {vm_size})"
    except:
...
data = {
    "summaryCards": [
        ...
        {"label": "Cost", "value": get_cost_estimate_azure(), "status": "info"}
    ],
    "vmInformation": [
        {"label": "Hostname", "value": os.environ.get('HOSTNAME_VM', 'unknown')},
        {"label": "Instance ID", "value": os.environ.get('INSTANCE_ID', 'unknown')},
        {"label": "VM Size", "value": os.environ.get('VM_SIZE', 'unknown')},
        {"label": "Location", "value": os.environ.get('LOCATION', 'unknown')},
        {"label": "Resource Group", "value": os.environ.get('RESOURCE_GROUP', 'unknown')},
        {"label": "Subscription ID", "value": os.environ.get('SUBSCRIPTION_ID', 'unknown')},
        {"label": "OS", "value": os.environ.get('OS_NAME', 'unknown')},
        {"label": "Image Offer", "value": os.environ.get('OFFER', 'unknown')},
        {"label": "Image SKU", "value": os.environ.get('SKU', 'unknown')}
    ],
    "services": [
    ...
    "meta": {
        "appName": os.environ.get('DASHBOARD_APP_NAME', 'Azure Dashboard'),
        "tagline": os.environ.get('DASHBOARD_TAGLINE', 'Azure Infrastructure Monitoring'),
        "dashboardUser": os.environ.get('DASHBOARD_USER', 'User'),
        "dashboardName": os.environ.get('DASHBOARD_NAME', 'Azure Dashboard'),
