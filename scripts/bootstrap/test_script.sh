...
data = {
    ...
    "vmInformation": [
        {"label": "Hostname", "value": f"{hostname or existing.get('identity', {}).get('hostname', 'unknown')}"},
        {"label": "Instance ID", "value": f"{os.environ.get('INSTANCE_ID') or existing.get('identity', {}).get('instanceId', 'unknown')}"},
        {"label": "Zone", "value": f"{zone or existing.get('location', {}).get('zone', 'unknown')}"},
        {"label": "Machine Type", "value": f"{os.environ.get('MACHINE_TYPE') or existing.get('identity', {}).get('machineType', 'unknown')}"},
        {"label": "OS", "value": f"{os.environ.get('OS_NAME', 'unknown')}"},
        {"label": "Project ID", "value": f"{os.environ.get('PROJECT_ID') or existing.get('identity', {}).get('project', 'unknown')}"},
    ...
    "security": [
        ...
        {"label": "Internal IP", "value": f"{internal_ip or existing.get('network', {}).get('internalIp', 'unknown')}", "status": "info"},
        {"label": "Public IP", "value": f"{public_ip or existing.get('network', {}).get('externalIp', 'unknown')}", "status": "info"}
    ],
    ...
    "meta": {
        ...
        "uptime": f"{uptime or existing.get('location', {}).get('uptime', 'unknown')}",
    },
    ...
    "identity": {
        "project": f"{os.environ.get('PROJECT_ID') or existing.get('identity', {}).get('project', 'unknown')}",
        "instanceId": f"{os.environ.get('INSTANCE_ID') or existing.get('identity', {}).get('instanceId', 'unknown')}",
        "hostname": f"{hostname or existing.get('identity', {}).get('hostname', 'unknown')}",
        "machineType": f"{os.environ.get('MACHINE_TYPE') or existing.get('identity', {}).get('machineType', 'unknown')}"
    },

    "network": {
        "vpc": "default",
        "subnet": f"{region}-subnet" if region != 'unknown' else existing.get('network', {}).get('subnet', 'unknown-subnet'),
        "internalIp": f"{internal_ip or existing.get('network', {}).get('internalIp', 'unknown')}",
        "externalIp": f"{public_ip or existing.get('network', {}).get('externalIp', 'unknown')}"
    },

    "location": {
        "region": f"{region or existing.get('location', {}).get('region', 'unknown')}",
        "zone": f"{zone or existing.get('location', {}).get('zone', 'unknown')}",
        "uptime": f"{uptime or existing.get('location', {}).get('uptime', 'unknown')}",
