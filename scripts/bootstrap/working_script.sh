...
# ------------------------------------------------------------
# Helper functions that fetch fresh data directly
# ------------------------------------------------------------
...
def get_zone():
    try:
        ...
        with urllib.request.urlopen(req, timeout=2) as resp:
            ...
            return full.split('/')[-1]  # e.g., "us-central1-a"
    except:
        return 'unknown'
...
def get_machine_type():
    try:
        ...
        with urllib.request.urlopen(req, timeout=2) as resp:
            full = resp.read().decode().strip()
            return full.split('/')[-1]  # e.g., "e2-micro"
    except:
        return 'unknown'
...
def get_uptime():
    try:
        # Use uptime -p for reliable human-readable format
        result = subprocess.check_output(['uptime', '-p'], text=True, stderr=subprocess.DEVNULL)
        # Output looks like "up 1 hour, 2 minutes" or "up 3 days, 4 hours, 5 minutes"
        return result.strip()
    except:
        try:
...
def get_ssh_status():
    try:
        status = subprocess.check_output(['systemctl', 'is-active', 'ssh'], text=True).strip()
        return "Enabled (22/tcp)" if status == 'active' else "Disabled"
    except:
        return "Disabled"
...
def get_update_status():
    try:
        ...
        updates = len([l for l in lines if l and not l.startswith('Listing')])
        if updates == 0:
            return "Up to date"
        elif updates < 5:
            return f"{updates} security updates"
        else:
...
def get_services_status():
    nginx = "Running" if subprocess.call(['systemctl', 'is-active', '--quiet', 'nginx']) == 0 else "Stopped"
    python = "Installed" if subprocess.call(['which', 'python3'], stdout=subprocess.DEVNULL) == 0 else "Missing"
    return nginx, python
...
nginx_status, python_status = get_services_status()
    ...
    try:
        with open('/proc/meminfo', 'r') as f:
            ...
            for line in f:
                if ':' in line:
                    key, val = line.split(':', 1)
                    meminfo[key] = int(val.strip().split()[0]) / 1024  # MB
...
# Logs (sample with current time)
logs = [
    {"time": datetime.now().strftime("%H:%M:%S"), "level": "info", "scope": "system", "message": "Dashboard initialized"},
    {"time": (datetime.now() - timedelta(minutes=5)).strftime("%H:%M:%S"), "level": "info", "scope": "metrics", "message": f"CPU: {cpu_usage}%, Memory: {mem_percent}%"},
...
resource_table = [
    ...
    {"name": "python3", "type": "runtime", "scope": "system", "status": python_status},
...
data = {
    "summaryCards": [
        {"label": "CPU", "value": f"{cpu_usage}%", "status": status_color(cpu_usage)},
        {"label": "Memory", "value": f"{mem_percent}%", "status": status_color(mem_percent)},
        {"label": "Disk", "value": f"{disk_percent}%", "status": status_color(disk_percent)},
    ...
    "services": [
        {"label": "Nginx", "value": nginx_status, "status": "healthy" if nginx_status == "Running" else "warning"},
