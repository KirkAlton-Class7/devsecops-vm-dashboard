...
```bash
SHARED_QUOTES_FILE="${REPO_DIR}/shared/assets/quotes/quotes.json"
SHARED_GALLERY_DIR="${REPO_DIR}/shared/assets/images/image_gallery"
SHARED_GALLERY_MANIFEST="${SHARED_GALLERY_DIR}/gallery-manifest.json"
```

* **`SHARED_QUOTES_FILE`** – Source quote dataset copied to `/var/www/vm-dashboard/data/quotes.json`.
* **`SHARED_GALLERY_DIR`** – Source WebP gallery image directory copied to `/var/www/vm-dashboard/data/images/`.
* **`SHARED_GALLERY_MANIFEST`** – Source gallery metadata copied to `/var/www/vm-dashboard/data/gallery-manifest.json`.
