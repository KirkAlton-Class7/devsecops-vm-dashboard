...
DASHBOARD_ROOT="${REPO_DIR}/dashboard-advanced"
...
sync_shared_assets() {
  ...
  elif [ -f "${DATA_DIR}/quotes_local.json" ] && [ ! -f "${DATA_DIR}/quotes.json" ]; then
    cp -f "${DATA_DIR}/quotes_local.json" "${DATA_DIR}/quotes.json"
    log "Fallback local quotes applied"
  else
  ...
  if [ -d "${SHARED_GALLERY_DIR}" ]; then
    ...
    log "WARN: Shared gallery directory not found at ${SHARED_GALLERY_DIR}"
  fi
  ...
  if [ -f "${SHARED_GALLERY_MANIFEST}" ]; then
    ...
    log "WARN: Shared gallery manifest not found at ${SHARED_GALLERY_MANIFEST}"
  fi
...
if [ -d "$SCRIPT_SOURCE_DIR" ]; then
    cp -rf "$SCRIPT_SOURCE_DIR/"* /opt/scripts/ 2>/dev/null
...
# ---------------------------------
log "Shared quotes are deployed from repository assets; external quote cron disabled"
(crontab -l 2>/dev/null | grep -v 'vm-dashboard-sync') | crontab -
...
cd "$APP_SOURCE_DIR" || { log "ERROR: dashboard dir missing at ${APP_SOURCE_DIR}"; exit 1; }
chown -R ${APP_USER}:${APP_USER} "$REPO_DIR"
...
if [ ! -d "$APP_SOURCE_DIR/dist" ] || [ ! -f "$APP_SOURCE_DIR/dist/index.html" ]; then
...
sync_shared_assets() {
    ...
    if [ -d "$SHARED_GALLERY_DIR" ]; then
        find "$DATA_DIR/images" -mindepth 1 -maxdepth 1 -type f -delete 2>/dev/null || true
        find "$SHARED_GALLERY_DIR" -maxdepth 1 -type f \( -name '*.webp' -o -name '*.jpg' -o -name '*.jpeg' -o -name '*.png' \) -exec cp -f {} "$DATA_DIR/images/" \;
        log "Shared gallery images applied"
    fi

    if [ -f "$SHARED_GALLERY_MANIFEST" ]; then
        cp -f "$SHARED_GALLERY_MANIFEST" "$DATA_DIR/gallery-manifest.json"
        cp -f "$SHARED_GALLERY_MANIFEST" "$DATA_DIR/images.json"
        log "Shared gallery manifest applied"
    fi
...
if [ "$LOCAL" != "$REMOTE" ]; then
    ...
    cd "$APP_SOURCE_DIR" || { log "Dashboard directory missing at $APP_SOURCE_DIR"; exit 1; }

    if [ -f "$SCRIPT_SOURCE_DIR/fetch_pricing.py" ]; then
        mkdir -p /opt/scripts
        cp -f "$SCRIPT_SOURCE_DIR/fetch_pricing.py" /opt/scripts/fetch_pricing.py
        chmod +x /opt/scripts/fetch_pricing.py
