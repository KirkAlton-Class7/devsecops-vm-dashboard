...
# Fetch Python Script
# -------------------------------
log "Fetching generate-data.py"

aws s3 cp s3://your-bucket/scripts/generate-data.py /opt/generate-data.py || {
  log "ERROR: Failed to fetch generate-data.py"
  exit 1
}

# Validate file exists BEFORE using it
if [ ! -f /opt/generate-data.py ]; then
  log "ERROR: generate-data.py not found after download"
  exit 1
fi

chmod +x /opt/generate-data.py
...
DATA_DIR="${APP_DIR}/data" python3 /opt/generate-data.py
