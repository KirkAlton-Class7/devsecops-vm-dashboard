...
REPO_URL="$(metadata_value dashboard-repo-url)"
LETSENCRYPT_STAGING="${LETSENCRYPT_STAGING:-false}"
REPO_URL="${REPO_URL:-https://github.com/KirkAlton-Class7/devsecops-vm-dashboard.git}"
...
if [ ! -d "$REPO_DIR" ]; then
    ...
    timeout 120 git clone --depth 1 "$REPO_URL" "$REPO_DIR"
else
...
# ------------------------------------------------------------
MAIN_SCRIPT="/opt/deploy/dashboard-basic/scripts/bootstrap/app_bootstrap.sh"
...
MONITORING_SCRIPT="/opt/deploy/dashboard-basic/scripts/monitoring_server.py"
...
Type=simple
User=root
WorkingDirectory=/opt/deploy/dashboard-basic/scripts
ExecStart=/usr/bin/python3 /opt/deploy/dashboard-basic/scripts/monitoring_server.py
Restart=always
